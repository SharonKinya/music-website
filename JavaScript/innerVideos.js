...
function displayPlayingVideos() {
// const videosContainer = document.getElementById("videos");

videos.forEach((videoItem, index)  => {
    const newVideoElement = document.createElement('div');
    newVideoElement.classList.add('page-entity');
    // videoElement.setAttribute('id', videoItem.id);
    const videoDetails = `
     <div class="container">
        <div class="main-video">
            <div class="video">
                <video src='${videoItem.video}'controls autoplay></video>
                <h3 class="title">${videoItem.id} ${videoItem.artist} - ${videoItem.name}</h3>
            </div>
        </div>
