    ...
    const videoDetails = `
     <div class="card-container">
            <div class="card">
                ...
                <div class="play-button" id="videoPlaying${videoItem.id}" onclick="toggleVideoPlay('${videoItem.id}')" >
                    <i class="fa-solid fa-play" class="play-icon" id="icon1"></i> 
                    <div class="tooltip">play</div>
                </div>

                <div class="video-player>
                <video id="myVideo${videoItem.id}"> 
         ...
         getElement.appendChild(newVideoElement);
});
}

displayVideos();

function toggleVideoPlay(videoItemId) {
    const videoIdElement = document.getElementById(`videoPlaying${videoItemId}`);
    const myVideoElement = document.getElementById(`myVideo${videoItemId}`);
    
       if(myVideoElement.paused){
        myVideoElement.play();
        videoIdElement.innerHTML =  `<i class="fa-solid fa-pause" class="play-icon" id="icon1"></i>`;
       } else {
        myVideoElement.pause();
        videoIdElement.innerHTML =  `<i class="fa-solid fa-play" class="play-icon" id="icon1"></i>`;
       };
    };
